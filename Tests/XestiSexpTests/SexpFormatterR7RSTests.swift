...
extension SexpFormatterR7RSTests {
    ...
    func test_format_bytevector() throws {
        #expect(try _fmt(Sexp(bytevector: [])) == "#u8()")
        #expect(try _fmt(Sexp(bytevector: [255])) == "#u8(255)")
        #expect(try _fmt(Sexp(bytevector: [1, 2, 3])) == "#u8(1 2 3)")
    }

    @Test
    func test_format_character() throws {
        #expect(try _fmt(Sexp(character: "z")) == "#\\z")
        #expect(try _fmt(Sexp(character: "\u{07}")) == "#\\alarm")
        #expect(try _fmt(Sexp(character: "\u{0a}")) == "#\\newline")
        #expect(try _fmt(Sexp(character: "\u{a0}")) == "#\\xa0")
    ...
    func test_format_symbol() throws {
        #expect(try _fmt(Sexp(symbol: "")) == "||")
        #expect(try _fmt(Sexp(symbol: "Frodo")) == "Frodo")
        #expect(try _fmt(Sexp(symbol: "Frodo Baggins")) == "|Frodo Baggins|")
        #expect(try _fmt(Sexp(symbol: "Frodo | \"Bäggins\"")) == "|Frodo \\| \"Bäggins\"|")
    }

    @Test
    func test_format_vector() throws {
        #expect(try _fmt(Sexp(vector: [])) == "#()")
